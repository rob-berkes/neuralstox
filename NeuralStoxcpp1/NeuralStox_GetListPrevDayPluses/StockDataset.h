...
public:
    ...
    StockClass *Alist;  //used in find_stock_dated to improve search time
    Neurons *matchList;
    StockClass *origin;
    string outfname;
    string tenabfname;
...
StockDS::StockDS(){
    ...
    this->matchList = new Neurons;
    this->origin = this->stockList;
    this->outfname = "/data/y09h01e2.csv";
    this->tenabfname = "";
    this->readFromFile();
}
StockDS::~StockDS() {
...
int StockDS::writeToTenABFile() {
    ...
    file.open(this->tenabfname.c_str(),std::fstream::out);
    int count=0;
...
int StockDS::writeToFile(){
    std::fstream file;
    StockClass *ptr = this->stockList;
    file.open(this->outfname.c_str(),std::fstream::out);
    int count=0;
    while (ptr!= NULL){
        string line;
...
int StockDS::readFromFile()
    {
        ifstream   file (this->outfname.c_str());
        string value;
        string sdate;
        string close;
        string change;
